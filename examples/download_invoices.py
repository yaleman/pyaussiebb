...
async def main():
    ...
    for filepath in config_files:
        if Path(filepath).expanduser().exists():
            configfile = AussieBBConfigFile.parse_file(Path(filepath).expanduser().resolve())
            break
    ...
    async with ClientSession() as session:
        aussiebb = AussieBB(username=configfile.users[0].username, password=configfile.users[0].password, session=session)
        print("Logging in...")
        ...
        for _transaction_date, transaction in transactions.items():
            for data in transaction:
                timestamp = datetime.strptime(data['time'], "%Y-%m-%d")
                download_path = Path(f"{data['time']}-{data['id']}-{data['type']}.pdf")
                ...
                if data['type'] == 'receipt':
                    invoice = await aussiebb.billing_receipt(data['id'])
                elif data['type'] == "invoice":
                    invoice = await aussiebb.billing_invoice(data['id'])
                elif data['type'] == "credit":
                    invoice = await aussiebb.billing_download(data['type'], data['id'])
                else:
                    print("Unhandled event type: %s for id=%s", data['type'], data['id'])
                    return
