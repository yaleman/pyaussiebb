...
def main():
    ...
    for service in services:
        ...
        if 'ipAddresses' not in service:
            continue
        client.logger.info("Found a service: {}", service.get('description'))
        for address in service["ipAddresses"]:
            client.logger.debug("address: {}", address)
            try:
                parsed = ip_network(address)
            except Exception as error_message: # pylint: disable=broad-except
                client.logger.error("Not sure what this was, but it's not an address! {} - {}", address, error_message)
                continue
