        ...
        if 'ipAddresses' in service:
            ...
            for address in service["ipAddresses"]:
                ...
                try:
                    ...
                    api.logger.error("Not sure what this was, but it's not an address! {} - {}", address, error_message)
                    continue
                if isinstance(parsed, IPv6Network):
                    if found_network is None:
                        found_network = parsed
                    if found_network.prefixlen > parsed.prefixlen:
                        api.logger.debug("Found bigger network, making it current: {}", parsed)
                        found_network = parsed
                    else:
    ...
    for line in terraform_file:
        ...
        if matches:
            ...
            if found_address != updated_address:
                api.logger.info("Need to update line... was {}, needs to be {}", found_address, updated_address)
                return True
