...
testfile = Path(os.path.join("/".join(__file__.split("/")[:-1]), "test_hardware.json")).read_text(encoding="utf-8")
...
async def test_hardware() -> None:
    ...
    filtered_services = client.filter_services(USAGE_ENABLED_SERVICE_TYPES, drop_unknown_types=True)
